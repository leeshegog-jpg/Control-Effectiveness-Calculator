...
| Status | **Resolved (2026-08-12), scope decided; implementation not authorized.** ACR required and not yet raised (ADR-006 §10); "OSR"/`osr_notified` remains a residual open item (ADR-006 §11). |
| Traceability | 3A §7, §12 D6; [20-r1-milestone-3c-d6-notification-evidence-matrix.md](20-r1-milestone-3c-d6-notification-evidence-matrix.md) (evidence); [ADR-006](../../.adr/ADR-006-incident-notification-rule-formal-defer.md) (formal defer, then resolution + ACR assessment). |
...
**Resolution order as executed:** D1 (closed) → D7 (accepted) → D2 (**Accepted, ADR-003**) → D4 (**Approved + contract-implemented, ACR-004**) → D3 (**Accepted, ADR-004**) → D5 (**Accepted, ADR-005**) → **D6** (**Deferred then Resolved, ADR-006** — scope decided; ACR required and not yet raised; OSR/`osr_notified` residual open). **All seven decision points now have a recorded disposition; D6 is the only one with a follow-on ACR still outstanding.**
...
| D6 | ADR (scope decision) — **[ADR-006](../../.adr/ADR-006-incident-notification-rule-formal-defer.md), Resolved 2026-08-12** — plus **ACR required, not yet raised** | Scope resolved; extending R10's definition in `07-inference-rules-catalogue.md` (and any new trigger-flag column) is a Design Baseline artefact change requiring ACR, same class as D4/`10-openapi.yaml` |
...
**Two ACRs are now implicated:** D4 — [ACR-004](../../.acr/ACR-004-incident-openapi-extension.md), **Approved and contract-implemented 2026-08-11**. D6 — a second ACR, identified by ADR-006 §10 as required to extend R10's definition in `07-inference-rules-catalogue.md`, **not yet raised**. **No ACR was required or raised for D2, D3, or D5 — all resolved via ADR only.**
...
- ~~D6~~ — **Resolved (scope), ACR pending.** [ADR-006](../../.adr/ADR-006-incident-notification-rule-formal-defer.md) — deferred 2026-08-12, resolved same day: `whsq_notified` = internal, R10 extended to general WHS Act categories, Safety Systems Manager/48hr/10yr. **An ACR is required to extend `07-inference-rules-catalogue.md`'s R10 definition — not yet raised.** "OSR" meaning unanswered; `osr_notified` explicitly untouched, residual open item (ADR-006 §11).
