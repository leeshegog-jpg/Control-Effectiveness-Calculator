...
| IB21 | [R1 Milestone 3D — Incident Implementation Readiness & Scope](21-r1-milestone-3d-incident-implementation-readiness.md) | Reconciles the now-frozen Incident baseline (ADR-003/004/005/006, ACR-004/005, R23, `is_notifiable_incident`) against the verified R0 code state — confirms DTO/router/repository/service/ORM model/Neo4j-sync/tests are all placeholder-only or absent, `packages/shared-types` is stale against the Incident OpenAPI surface. Proposes an 8-slice implementation breakdown (3D-1 Incident persistence through 3D-7 tests/shared-types), recommends dropping/re-scoping a proposed "Safety Case Demonstration evidence" slice (no baseline artefact currently links `Incident` to `Demonstration`/`SafetyCaseClaim`), and tracks OSR as a non-blocking external dependency | **Complete, unmerged** (PR #18) — research/reconciliation only; authorizes no implementation; each slice requires its own explicit GO | IB18, IB19, IB20, 13 (application-foundation-scaffold), 02/06/07/09/10/11 (knowledge-graph) |
