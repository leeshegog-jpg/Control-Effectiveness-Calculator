...
|---|---|---|---|---|---|
| [ACR-004](../../.acr/ACR-004-incident-openapi-extension.md) | Incident Domain — OpenAPI Extension (Investigation, `incident_hazards`/`REVEALS`, incident-scoped Evidence) | 2026-08-09 | **2026-08-11** | [IB19](19-r1-milestone-3b-incident-decision-register.md) D4; [ADR-003](../../.adr/ADR-003-incident-investigation-action-sibling-structure.md) | `10-openapi.yaml` only, additive-only — no schema/Neo4j/ontology/code change proposed |

Distinct from the Closed table above: ACR-002/003 were approved **and** their artefact regeneration was completed in the same governance pass (IB14 §6–§8). **ACR-004 is approved but `10-openapi.yaml` remains unchanged** — per explicit instruction accompanying this ACR's approval, editing the contract and any subsequent implementation both require a separate, further GO (ACR-004 §17–§18). Design Baseline stays **v1.1** until that edit is made.
