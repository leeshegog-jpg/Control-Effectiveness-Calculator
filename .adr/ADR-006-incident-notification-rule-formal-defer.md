...
## 11. Residual Open Item

**"OSR" meaning (§6 Q1) remains unanswered.** `osr_notified` and R10's existing Chapter-9A behaviour are unaffected by this determination and stay exactly as previously specified. This is not blocking for the whsq_notified/general-WHS-Act extension the determination did resolve (§9), and is not blocking for Investigation/hazard-linking/incident-Evidence (ACR-004, unaffected throughout). It does block ever extending Q3–Q6's parameters to `osr_notified`, and blocks fully closing D6 end-to-end.

## 12. Status

**Resolved (2026-08-12), with one residual open item (§11) and one required-but-not-yet-raised ACR (§10).** D6's governance disposition: determination received and recorded; scope decision made (extend R10 to general WHS Act categories, `whsq_notified` = internal, 48hr/Safety Systems Manager/10yr retention); `osr_notified`/OSR meaning explicitly carved out as still open. No ACR raised, no schema/OpenAPI/code change made by this document. Implementation remains separately gated — this ADR authorizes a requirement, not a build.
