...
- **[ADR-007](ADR-007-contract-test-suite-scope-and-ci-treatment.md)** — OpenAPI contract test suite (`tests/contract/`, Schemathesis, merged inactive in PR #44): strict conformance is asserted against the 46 implemented operations only; the 67 deferred operations stay visible as runtime-classified `skip`s, not removed; no fake-token auth testing — Schemathesis auth-conformance checks are disabled and the `bearerAuth`-declared-but-not-enforced discrepancy is recorded as risk-register S4; the suite runs as its own non-required, report-only CI job; the branch ruleset is unchanged; explicit promotion criteria gate any future move to a blocking/required check. Supersedes the current one-line `ci/wire-contract-tests` approach. Accepted 2026-08-30 — implementation slice and report-only CI wiring remain separate, each its own GO.
...
| [ADR-007](ADR-007-contract-test-suite-scope-and-ci-treatment.md) | OpenAPI Contract Test Suite — Scope, Failure Semantics, and CI Treatment | Accepted — implementation + report-only CI wiring pending, each its own GO |
